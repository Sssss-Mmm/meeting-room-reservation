...
| 빈 방 찾기 | 시간대·인원을 입력하면 그때 예약 가능한 회의실만 조회 (작은 방 우선) |
| 예약 캘린더 | 월별 내 예약 현황 캘린더 뷰 |
| 예약 승인/반려 | 관리자가 대기 중인 예약을 처리 |
| 알림 | 예약 상태 변경 시 알림 표시 |
| 회의실 관리 | 관리자가 회의실 추가·활성화/비활성화, 승인 정책 설정 |
...
### 2. 예약 승인 워크플로 — 회의실별 승인 정책

모든 예약을 관리자 승인 대기로 두면 "30분 뒤 회의"를 잡을 수 없어 실사용에서 앱을 우회하게 됩니다. 그래서 승인 여부를 회의실 속성(`Room.needsApproval`)으로 내렸습니다.

- `needsApproval = false` (기본): 예약 즉시 `CONFIRMED`
- `needsApproval = true`: `PENDING`으로 생성되고 관리자에게 알림 → `CONFIRMED / REJECTED`

대회의실·임원회의실처럼 통제가 필요한 방만 승인제로 두고 나머지는 즉시 확정합니다. 정책은 관리자 화면에서 회의실별로 토글할 수 있습니다. 사용자는 어느 경우에도 `CANCELLED`로 전환할 수 있고, 이미 시작된 예약은 취소할 수 없습니다.

> 기존 DB에는 `rooms.needs_approval` 컬럼이 `ddl-auto: update`로 추가되며, 기존 회의실은 모두 즉시 확정(false)이 됩니다.
